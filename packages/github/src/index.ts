...
async function getExistingFileSha(input: {
  ...
  try {
    ...
    return getContentFile(response.data)?.sha;
  } catch (error) {
    if (isNotFoundError(error)) {
      return undefined;
    }
...
export async function createContextPr(input: {
  ...
  const modelProvider =
    input.modelProvider ?? writableArtifacts[0]?.artifact.modelProvider ?? null;
  const model = input.model ?? writableArtifacts[0]?.artifact.model ?? null;
  ...
  return {
    ...
    artifactVersions: writableArtifacts.map(({ artifact }) => artifact.version),
    status: "succeeded",
    createdAt: nowIso(),
  };
}
